...
function computeQualityScore(record: SleepRecord): number {
  if (record.stages) {
...
function evaluateWindow(
  ...
  const residuals = points.map((p) =>
    Math.abs(p.y - (slope * p.x + intercept)),
  );
...
export function analyzeCircadian(records: SleepRecord[], extraDays = 0): CircadianAnalysis {
  ...
  const abDates = [
    ...new Set(
      candidates.filter((c) => c.tier !== "C").map((c) => c.record.dateOfSleep),
    ),
  ].sort();
  ...
  for (let i = 1; i < abDates.length; i++) {
    const gap =
      (new Date(abDates[i]! + "T00:00:00").getTime() -
        new Date(abDates[i - 1]! + "T00:00:00").getTime()) /
      86_400_000;
  ...
  for (const record of sorted) {
    ...
    anchors.push({
      dayNumber:
        (new Date(record.dateOfSleep + "T00:00:00").getTime() - firstDateMs) /
        86_400_000,
  ...
  const lastDay = Math.max(
    ...
    (new Date(sorted[sorted.length - 1]!.dateOfSleep + "T00:00:00").getTime() -
      firstDateMs) / 86_400_000,
  );
  ...
  const medResidual =
    allResiduals.length > 0
      ? allResiduals[Math.floor(allResiduals.length / 2)]!
      : 0;
