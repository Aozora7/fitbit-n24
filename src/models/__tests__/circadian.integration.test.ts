...
describe("regression-v1 specific", () => {
    const analyze = (records: Parameters<typeof analyzeWithAlgorithm>[1], extraDays?: number) =>
        analyzeWithAlgorithm("regression-v1", records, extraDays) as RegressionAnalysis;

    it("returns sane defaults for empty input (anchors)", () => {
        const result = analyze([]);
    ...
    it("tiny segment (< 2 anchors) is skipped gracefully", () => {
        ...
        const shiftedTiny = shiftRecords(tiny, baseTime, 110);
...
describe.skipIf(!hasRealData(AOZORA_FILE))("regression-v1 — real data regression", () => {
    it("Sept 2022 fragmented sleep does not reverse overlay direction", () => {
        const records = loadRealData(AOZORA_FILE);
        const result = analyzeWithAlgorithm("regression-v1", records) as RegressionAnalysis;

        const septDays = result.days.filter((d) => d.date >= "2022-09-14" && d.date <= "2022-09-22");
        ...
        expect(totalShift).toBeGreaterThan(-4);
