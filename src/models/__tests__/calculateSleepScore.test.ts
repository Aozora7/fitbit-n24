...
describe("calculateSleepScore", () => {
  ...
  it("short sleep (2h) scores lower than normal sleep (8h)", () => {
    const short = calculateSleepScore(makeRawRecord({
      minutesAsleep: 120,
  ...
  it("high wake percentage lowers score", () => {
    const lowWake = calculateSleepScore(makeRawRecord({
      levels: {
        data: [],
        shortData: [],
        summary: {
    ...
    const highWake = calculateSleepScore(makeRawRecord({
      levels: {
        data: [],
        shortData: [],
        summary: {
  ...
  it("score is always clamped to [0, 1]", () => {
    ...
    const extreme = calculateSleepScore(makeRawRecord({
      minutesAsleep: 0,
