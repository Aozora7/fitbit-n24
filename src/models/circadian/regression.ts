...
export function weightedLinearRegression(points: { x: number; y: number; w: number }[]): { slope: number; intercept: number } {
    let sumW = 0,
        sumWX = 0,
        sumWY = 0,
        sumWXX = 0,
...
export function robustWeightedRegression(points: { x: number; y: number; w: number }[], maxIter = 5, tuningConstant = 4.685): { slope: number; intercept: number } {
    if (points.length < 2) {
...
export function evaluateWindow(anchors: Anchor[], centerDay: number, halfWindow: number, sigma: number = GAUSSIAN_SIGMA): WindowResult {
