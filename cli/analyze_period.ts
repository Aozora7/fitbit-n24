 ...
 * Usage: npx tsx cli/analyze_period.ts <sleep-data.json> [startDate] [endDate] [algorithmId]
 * Example: npx tsx cli/analyze_period.ts data.json 2023-10-14 2023-10-26
 * Example: npx tsx cli/analyze_period.ts data.json 2023-10-14 2023-10-26 kalman-v1
 */
...
import {
    ...
    listAlgorithms,
    _internals,
} from "../src/models/circadian/index.js";
...
const algorithmId = process.argv[5] || DEFAULT_ALGORITHM_ID;
...
const isRegression = algorithmId === "regression-v1";

console.log(`=== Period analysis: ${startDate} to ${endDate} (${algorithmId}) ===\n`);
console.log(
    `Global: tau=${analysis.globalTau.toFixed(4)}h, drift=${(analysis.globalDailyDrift * 60).toFixed(1)} min/day`
);
if (isRegression) {
    const regAnalysis = analysis as RegressionAnalysis;
    console.log(`Total anchors: ${regAnalysis.anchors.length}`);
}
console.log();

// Filter days in the target range
const targetDays = analysis.days.filter((d) => d.date >= startDate && d.date <= endDate && !d.isForecast);

// Regression-specific: show anchors in range
if (isRegression) {
    const regAnalysis = analysis as RegressionAnalysis;
    const targetAnchors = regAnalysis.anchors.filter((a) => a.date >= startDate && a.date <= endDate);
...
// Show raw sleep records in the range
...
// Regression-specific: show local regression details
if (isRegression) {
    const regAnalysis = analysis as RegressionAnalysis;
    ...
    const internalAnchors = regAnalysis.anchors.map((a) => ({
        ...a,
    ...
    for (const d of sampleDays) {
        ...
        console.log(
            `    Centroid (weightedMeanX): day ${result.weightedMeanX.toFixed(1)} (offset=${(d - result.weightedMeanX).toFixed(1)})`
        );

        const windowAnchors = regAnalysis.anchors
